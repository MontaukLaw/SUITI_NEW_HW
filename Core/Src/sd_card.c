...
void SD_SendCmdRaw(uint8_t cmd, uint32_t arg, uint8_t crc)
{
    /* 命令格式：
     * [0] 0b01xxxxxx | cmd
     * [1] arg[31:24]
    ...
    SD_SPI_TxRx(crc | 0x01); // bit0 必须为 1
}
...
uint8_t Read_Single_Block_(uint32_t block, uint8_t *data)
{
    uint8_t r1;
...
uint8_t SD_WaitR1(uint32_t timeout)
{
    uint8_t r1;
...
uint8_t SD_Reset_Card_OLD(void)
{
    uint8_t r1;
...
{
    uint8_t r1;
    uint16_t retry = 0x1FF;

    SD_CS_High();

    /* >= 80 clocks with MOSI high */
    for (int i = 0; i < 10; i++)
        SD_SPI_TxRx(0xFF);

    SD_CS_Low();

    SD_SendCmdRaw(0, 0, 0x95); // CMD0 + 正确 CRC
    ...
    do
    {
        ...
        delay_us(10);
        // } while (r1 == 0xFF && retry--);
    } while (r1 == 0xFF || r1 == 0x05);

    SD_CS_High();
    SD_SPI_TxRx(0xFF);

    return r1; // 正确值应为 0x01
}
...
bool sd_scan_step(void)
{
    ...
    static int32_t last_used = -1;
    sd_log_page_t temp;

    if (scan_idx >= SD_BlockNR)
        return true;

    if (Read_Single_Block(scan_idx, (uint8_t *)&temp) != SD_OK)
        return true;

    if (temp.magic != SD_LOG_MAGIC)
    {
        DBG_PRINTF("Last used block: %ld\r\n", last_used);
        DBG_PRINTF("Next free block: %lu\r\n", scan_idx);
        return true;
    }
...
void sd_look_for_avliable_block(void)
{
    ...
    for (uint32_t i = 0; i < SD_BlockNR; i++)
    {
        ...
        HAL_IWDG_Refresh(&hiwdg);
        if (Read_Single_Block(i, (uint8_t *)&temp_page) != SD_OK)
        {
            DBG_PRINTF("Read block %lu failed\r\n", i);
            return;
        }

        if (temp_page.magic != SD_LOG_MAGIC)
        {
            DBG_PRINTF("Block %lu: invalid magic: 0x%08lX\r\n", i, temp_page.magic);
            break;
        }
    ...
    {
        DBG_PRINTF("Last used block: %ld\r\n", last_used_block);
        DBG_PRINTF("Next available block: %ld\r\n", last_used_block + 1);
    }
    else
    {
        DBG_PRINTF("No used blocks found\r\n");
    }
}
...
void read_test(void)
{
    ...
    // printf
    DBG_PRINTF("Magic: 0x%08lX\r\n", sd_log_page.magic);
    DBG_PRINTF("Seq: %lu\r\n", sd_log_page.seq);
    DBG_PRINTF("Timestamp: %lu\r\n", sd_log_page.timestamp);
    DBG_PRINTF("CRC: 0x%08lX\r\n", sd_log_page.crc);
